  ...
  const LoginPage = ( { isAuthenticated, setIsAuthenticated }) => {
    ...
    var UserIdFeild = <TextField autoFocus fullWidth id='userid' label='User ID' sx={{ marginBottom: 4}} value={userid} onChange={(e) => setUserid(e.target.value)}/>
    ...
    const userSessionCall = (session) => {
      ...
      fetch(`http://192.168.1.109:3001/api/userSession?${new URLSearchParams(query)}`, {
        method: 'POST',
        headers: {
    ...
    const handleLoginClick = (request, session) => {
      ...
      fetch(`http://192.168.1.109:3001/api/auth?${new URLSearchParams(query)}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      .then(data => {
        ...
        if (auth == 1){
          ...
          router.push(`/pages/dashboard?userSession=${session}`);
        }
        else{
          setTimeout(() => {
            UserIdFeild = <TextField error autoFocus fullWidth id='userid' label='User ID' sx={{ marginBottom: 4}} value={userid} onChange={(e) => setUserid(e.target.value)}/>;
            router.push('/');
          }, 500);
        }
      })
